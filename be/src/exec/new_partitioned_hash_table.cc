...
NewPartitionedHashTableCtx::NewPartitionedHashTableCtx(
        const std::vector<Expr*>& build_exprs, const std::vector<Expr*>& probe_exprs,
...
Status NewPartitionedHashTableCtx::Create(
        ...
        int num_build_tuples, MemPool* mem_pool, MemPool* expr_results_pool, MemTracker* tracker,
...
uint32_t NewPartitionedHashTableCtx::HashRow(const uint8_t* expr_values,
...
Status NewPartitionedHashTableCtx::ExprValuesCache::Init(RuntimeState* state, MemTracker* tracker,
    ...
    if (UNLIKELY(!tracker->try_consume(mem_usage))) {
        ...
        string details = Substitute(
                "NewPartitionedHashTableCtx::ExprValuesCache failed to allocate $0 bytes.",
                mem_usage);
...
uint8_t* NewPartitionedHashTableCtx::ExprValuesCache::ExprValuePtr(uint8_t* expr_values,
                                                                   int expr_idx) const {
    return expr_values + expr_values_offsets_[expr_idx];
}

const uint8_t* NewPartitionedHashTableCtx::ExprValuesCache::ExprValuePtr(const uint8_t* expr_values,
                                                                         int expr_idx) const {
    return expr_values + expr_values_offsets_[expr_idx];
}
...
NewPartitionedHashTable* NewPartitionedHashTable::Create(
        Suballocator* allocator, bool stores_duplicates, int num_build_tuples,
        BufferedTupleStream3* tuple_stream, int64_t max_num_buckets, int64_t initial_num_buckets) {
    return new NewPartitionedHashTable(config::enable_quadratic_probing, allocator,
                                       stores_duplicates, num_build_tuples, tuple_stream,
...
Status NewPartitionedHashTable::CheckAndResize(uint64_t buckets_to_fill,
                                               const NewPartitionedHashTableCtx* ht_ctx,
                                               bool* got_memory) {
    uint64_t shift = 0;
...
Status NewPartitionedHashTable::ResizeBuckets(int64_t num_buckets,
                                              const NewPartitionedHashTableCtx* ht_ctx,
                                              bool* got_memory) {
