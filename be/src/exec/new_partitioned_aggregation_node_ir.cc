...
void IR_ALWAYS_INLINE NewPartitionedAggregationNode::EvalAndHashPrefetchGroup(
        RowBatch* batch, int start_row_idx, NewPartitionedHashTableCtx* ht_ctx) {
    NewPartitionedHashTableCtx::ExprValuesCache* expr_vals_cache = ht_ctx->expr_values_cache();
    const int cache_size = expr_vals_cache->capacity();
...
template <bool AGGREGATED_ROWS>
Status NewPartitionedAggregationNode::AddIntermediateTuple(
        Partition* partition, TupleRow* row, uint32_t hash,
        NewPartitionedHashTable::Iterator insert_it) {
    while (true) {
...
Status NewPartitionedAggregationNode::ProcessBatchStreaming(
        bool needs_serialize, RowBatch* in_batch, RowBatch* out_batch,
...
bool NewPartitionedAggregationNode::TryAddToHashTable(NewPartitionedHashTableCtx* ht_ctx,
                                                      Partition* partition,
                                                      NewPartitionedHashTable* hash_tbl,
                                                      TupleRow* in_row, uint32_t hash,
