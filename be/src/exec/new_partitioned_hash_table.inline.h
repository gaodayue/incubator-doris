...
inline int64_t NewPartitionedHashTable::Probe(Bucket* buckets, int64_t num_buckets,
                                              NewPartitionedHashTableCtx* ht_ctx, uint32_t hash,
                                              bool* found) {
    DCHECK(buckets != NULL);
...
inline NewPartitionedHashTable::DuplicateNode* NewPartitionedHashTable::AppendNextNode(
        Bucket* bucket) {
...
inline TupleRow* IR_ALWAYS_INLINE NewPartitionedHashTable::GetRow(HtData& htdata,
                                                                  TupleRow* row) const {
    if (stores_tuples()) {
...
inline TupleRow* IR_ALWAYS_INLINE NewPartitionedHashTable::GetRow(Bucket* bucket,
                                                                  TupleRow* row) const {
    DCHECK(bucket != NULL);
