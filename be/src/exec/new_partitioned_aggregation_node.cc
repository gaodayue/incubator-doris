...
NewPartitionedAggregationNode::NewPartitionedAggregationNode(ObjectPool* pool,
...
Status NewPartitionedAggregationNode::prepare(RuntimeState* state) {
    ...
    if (!grouping_exprs_.empty()) {
        ...
        RETURN_IF_ERROR(NewPartitionedHashTableCtx::Create(
...
Status NewPartitionedAggregationNode::HandleOutputStrings(RowBatch* row_batch, int first_row_idx) {
...
Status NewPartitionedAggregationNode::GetNextInternal(RuntimeState* state, RowBatch* row_batch,
                                                      bool* eos) {
    SCOPED_TIMER(_runtime_profile->total_time_counter());
...
Status NewPartitionedAggregationNode::GetRowsStreaming(RuntimeState* state, RowBatch* out_batch) {
    DCHECK(!child_eos_);
    DCHECK(is_streaming_preagg_);
...
void NewPartitionedAggregationNode::CleanupHashTbl(const vector<NewAggFnEvaluator*>& agg_fn_evals,
...
void NewPartitionedAggregationNode::CopyGroupingValues(Tuple* intermediate_tuple, uint8_t* buffer,
                                                       int varlen_size) {
...
// TODO: codegen this function.
void NewPartitionedAggregationNode::InitAggSlots(const vector<NewAggFnEvaluator*>& agg_fn_evals,
...
void NewPartitionedAggregationNode::UpdateTuple(NewAggFnEvaluator** agg_fn_evals, Tuple* tuple,
...
Tuple* NewPartitionedAggregationNode::GetOutputTuple(const vector<NewAggFnEvaluator*>& agg_fn_evals,
...
Status NewPartitionedAggregationNode::AppendSpilledRow(Partition* partition, TupleRow* row) {
    DCHECK(!is_streaming_preagg_);
...
void NewPartitionedAggregationNode::DebugString(int indentation_level, stringstream* out) const {
...
Status NewPartitionedAggregationNode::CreateHashPartitions(int level, int single_partition_idx) {
...
template Status NewPartitionedAggregationNode::AppendSpilledRow<false>(Partition*, TupleRow*);
